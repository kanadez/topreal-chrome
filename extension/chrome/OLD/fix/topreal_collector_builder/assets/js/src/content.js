...
var host = null; // сюда пишется dev.topreal.top или topreal.top зависимо от флага тест_моде
var builder = new Builder(); // объект билдера коллекторов
var collector = new Collector(); // объект коллектора
...
var turned_on = 0; // тумблер всего расширения
var no_translate_mode = 0; 

chrome.extension.onMessage.addListener(function (request, sender, sendResponse) { // лисенер события включения от попапа
...
function toggleExtension(){ // включает/отключает расширение
    if (turned_on === 1){
        stopExtension();
    }
    else{
...
function startExtension(){ 
    ...
    $(document.body).append(
        '<div id="buttons_div" translate="no" title="TopReal Builder" style="display:none;text-align;center;">\n\
            ...
            <input style="width:100%" id="upload_collector_locale_input" type="file" name="file" accept=".csv" multiple="multiple" data-url="'+host+'/storage/upload.php" />\n\
            ...
            <button class="builder_button" id="edit_collector_button">Edit collector</button>\n\
            <button class="builder_button" id="refresh_editing_button">Refresh window</button>\n\
            <button class="builder_button" id="save_collector_button">Save collector</button>\n\
            <span id="save_success_span" style="display:none;color:green;">Successfully saved!</span>\n\
            <a id="collector_locale_download_a" href="'+host+'/storage/collector_locale.csv" style="display:none;">Download collector locale</a>\n\
            ...
            <button class="builder_button"  id="show_mst_button">Do action</button>\n\
            ...
            <div id="save_dialog_div" title="Set collector name" style="display:none;"><label for="collector_name_input">Collector name:</label><input id="collector_name_input" /><p><button id="save_created_collector_button">Save</button><p><span id="save_response_span"></span></div>\n\
    ...
    $('#save_created_collector_button').click(function(){
    ...
    $('#edit_collector_button').click(function(){
        builder.editCollector();
    });
    
    $('#save_collector_button').click(function(){
        builder.saveCollector();
    });
    
    $('#refresh_editing_button').click(function(){
...
function switchSelectMode(){ // включает режим выбора мышью, при котором все клики по странице обрабатываются (по-разному)
    if (select_mode === 1){
        select_mode = 0;
...
function switchSelectFieldsMode(){ // влючает режим выбора мышью для названий полей
    if (select_fields_mode === 1){
...
function switchSelectCheckboxMode(){ // включает режим выбора мыщью для чекбоксов
    if (select_checkbox_data_mode === 1){
...
function switchSelectXpathMode(){ // включает режим вывода значения xpath для выбранного элемента
